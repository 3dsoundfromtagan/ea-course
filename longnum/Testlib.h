...
/**
 * \brief Macros for outputting variables as a pair of key and value
 *
 * \details Macros for outputting variables as a pair of key and value
 *
 */
#define AS_KV(x)                                     \
    #x << " = " << x
...
#define ASSERT_EQUAL(x, y) {                         \
    ...
    os << #x << " != " << #y << " in"                \
       ...
       << ", function " << __func__                  \
       << ", line " << __LINE__;                     \
    Geo::Testlib::AssertEqual(x, y, os.str());       \
}
...
#define ASSERT_EQUAL_MSG(x, y, msg) {                \
    ...
    os << "message: " << msg << " "                  \
       ...
       << ", function " << __func__                  \
       << ", line " << __LINE__;                     \
    if (Geo::Testlib::AssertEqual(x, y, os.str()) && \
        Geo::Testlib::__assert_print_if_ok) {        \
        std::cerr << msg << " - OK\n";               \
    }                                                \
}
...
#define ASSERT_MSG(x, msg) {                         \
    ...
    os << "message: " << msg << " "                  \
       ...
       << ", function " << __func__                  \
       << ", line " << __LINE__;                     \
    if (Geo::Testlib::Assert(x, os.str()) &&         \
        Geo::Testlib::__assert_print_if_ok) {        \
        std::cerr << msg << " - OK\n";               \
    }                                                \
}
...
namespace Geo {
     ...
     *
     * \author Georgii Aloian aka VulpesCorsac
     *
     * \version V 1.0.0
     *
